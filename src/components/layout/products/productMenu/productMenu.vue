    ...
    <q-toolbar inverted color="dark" class="column">
      <div class="row items-center" style="width: 100%;height: 64px;border-bottom: 1px solid $border-color-1 ;">
        ...
        <q-tabs slot="navigation" class="productMenuTabBar">
            <q-route-tab style="font-weight:bold;" slot="title" :to="item.to" replace hide="icon" :label="item.tabName" v-for="(item, index) in tab" :key="item.id" :class="item.id===activeTab?'active':''" @click="activeTab=item.id"><span class="bottomLine"></span></q-route-tab>
        </q-tabs>
        <div>
